...
// NOTE: we don't call connectDB() immediately here because for local
// development we want to ensure the DB connection completes (or fails)
// before we start accepting requests — this makes startup ordering
// deterministic and produces clearer logs when something goes wrong.
// The local start path below calls `connectDB()` and awaits it.

// Attach global error handlers so unexpected rejections/exceptions
// are logged and visible during development.
...
process.on('uncaughtException', (err) => {
  ...
  // In production you might want to restart the process. For local
  // development we keep it simple and exit so the developer can inspect
...
app.delete('/.netlify/functions/server/images/:id', async (req, res) => {
  try {
    const image = await Image.findOne({ id: req.params.id }); // Change to findOne by id field
    if (!image) {
...
// Start local server in development — wait for DB connection first so
// startup logs are meaningful and the server won't accept requests
// before DB is ready.
if (process.env.NODE_ENV !== 'production') {
  const startLocalServer = async () => {
    try {
      console.log('Starting local server — attempting to connect to DB...');
      await connectDB();
      ...
      console.error('Failed to start local server:', err && err.message ? err.message : err);
      // Exit with non-zero so dev can see the failure in terminal
      process.exit(1);
    }
  };
  ...
  const gracefulShutdown = async () => {
    ...
    try {
      ...
      console.warn('Error during mongoose disconnect:', e && e.message ? e.message : e);
    }
    process.exit(0);
  };
